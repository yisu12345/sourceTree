//
//  QJZForgetPasswordViewController.m
//  去兼职
//
...
@interface QJZForgetPasswordViewController ()
{
    ...
    UIButton *getCodeBtn; //获取验证码
    NSString *phoneCodestr;       //获取的的手机验证码字符串
}
@end

@implementation QJZForgetPasswordViewController
...
-(void)setupMainUI:(CGFloat)startY
{
    ...
    NSArray *titleArray=@[@"请输入验证码",@"新密码",@"重复密码"];
...
-(void)navBtnAction:(UIButton *)btn
{
    ...
    if (btn.tag==3) {
        ...
        if (  [pwdField.text isEqualToString:surePwdField.text] ) {
            ...
            NSString *serverURL=[NSString stringWithFormat:@"%@public.php?action=updatePwd&moblie=%@&new_pwd=%@",kServer_URL, phoneNumberField.text, pwdField.text];
